...
- **`AgentLifecycle` persistence** (added post-5c-scaffold, after a live test caught the gap): `agent_lifecycle` table at schema v2 stores JSON-serialized lifecycle state per agent. Write-through on every transition in `update_lifecycle_for` + `admin_forfeit`. Hydration on boot in `RoutingCarrier::with_storage` — agents with persisted state restore it; agents without fall through to `AgentLifecycle::initial`. End-to-end test (`forfeit_persists_across_carrier_restart`) confirms Forfeit survives reopen — without this an attacker recovers from Forfeit by simply restarting the carrier, which would hollow out the entire stake-and-slash mechanic.
