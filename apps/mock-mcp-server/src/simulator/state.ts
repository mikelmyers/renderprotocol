...
// In-memory simulator state for the mock MCP server. The seed file holds
// fixed content (subjects, titles, etc.) plus offsets relative to "now"; the
// simulator turns those into ISO timestamps each time a tool is called so
// the demo always feels live.
//
// Six services are stitched together here. They share the same load step
// because v0 is one mock pretending to be many; if/when we split them into
// separate processes the seed stays the same shape — only the wiring changes.

const __dirname = dirname(fileURLToPath(import.meta.url));
// Lives under fixtures/ rather than data/ — the repo's .gitignore ignores
// data/ for runtime files (audit DB, etc.), and the seed is source-controlled
